...
public class Display_NodesHost : MonoBehaviour
{
    class Panel
	{
	...
	private void Update()
	{
		...
		foreach(var loc in Map_NodeHost.NodeLocation.List.ToList())
		{
		...
		foreach(var p in List.ToList())
		{
			if(!(from loc in Map_NodeHost.NodeLocation.List select loc.guid).Contains(p.GUID))
			{
