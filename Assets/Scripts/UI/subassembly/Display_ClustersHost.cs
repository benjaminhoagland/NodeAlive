...
public class Display_ClustersHost : MonoBehaviour
{
    class Panel
	{
	...
	private void Update()
	{
		...
		foreach(var loc in Map_ClusterHost.ClusterLocation.List.ToList())
		{
		...
		foreach(var p in List.ToList())
		{
			if(!(from loc in Map_ClusterHost.ClusterLocation.List select loc.guid).Contains(p.GUID))
			{
